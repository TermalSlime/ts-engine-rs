...
use std::{ffi::{c_void, CString}, mem, ptr, str};
...
impl ShaderProgram
{
    pub fn link_program(vs: &VertexShader, fs: &FragmentShader) -> Option<ShaderProgram>
    {
        unsafe
        {
            ...
            return Some( ShaderProgram { ptr: program, ..Default::default()});
        }
    }
    ...
    pub fn apply_shader_attributes(&self)
    {
        ...
        for attr in &self.attributes
        {
            unsafe
            {
                let attr_ptr = GetAttribLocation(self.ptr, CString::new(attr.name.clone()).unwrap().as_ptr());
